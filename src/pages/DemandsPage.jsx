...
function DemandsPage() {
  ...
  // Users ve essences abonelikleri
  useEffect(() => {
  ...
  // Talep aboneliği - 250 gram katları içinde kalanları göster
  useEffect(() => {
    ...
    const unsubscribeDemands = subscribeToDemands((demands) => {
      ...
      for (const demand of demands) {
        ...
        if (!essence || demand.totalDemandBefore === undefined) continue;

        const completedBuckets = Math.floor(essence.totalDemand / 250);
        const displayThreshold = completedBuckets * 250;

        if (demand.totalDemandBefore >= displayThreshold) continue;
